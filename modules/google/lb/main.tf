...
locals {
  address      = var.create_address ? join("", google_compute_global_address.default.*.id) : var.address
  ipv6_address = var.create_ipv6_address ? join("", google_compute_global_address.default_ipv6.*.id) : var.ipv6_address

  url_map             = var.create_url_map ? join("", google_compute_url_map.default.*.id) : var.url_map
...
resource "google_compute_global_forwarding_rule" "http_ipv6" {
  depends_on = [
    google_compute_global_address.default_ipv6,
  ]
  provider              = google-beta
  project               = var.project
...
resource "google_compute_target_http_proxy" "default" {
  ...
  url_map = var.https_redirect == false ? local.url_map : join("", google_compute_url_map.https_redirect.*.id)
}
...
resource "google_compute_url_map" "default" {
  ...
  default_service = local.backends[var.url_map_config.default_backend].id

  dynamic "host_rule" {
    for_each = var.url_map_config.host_rules
    content {
      hosts        = host_rule.value.hosts
      path_matcher = host_rule.value.path_matcher.name
    }
  }
  ...
  dynamic "path_matcher" {
    ...
    iterator = host_rule
    content {
      name            = host_rule.value.path_matcher.name
      default_service = local.backends[host_rule.value.path_matcher.default_backend].id
      dynamic "path_rule" {
        for_each = host_rule.value.path_matcher.path_rules
        content {
