data "aws_lambda_function" "development_backend" {
  function_name = "development-${local.project_name}-backend"
}
...
module "development_api_gateway" {
  ...
  integrations = {
    ...
    "GET /app/{file+}" = {
      description          = "S3 frontend proxy"
      integration_type     = "HTTP_PROXY"
      integration_uri      = "http://${module.s3_bucket_development_frontend["${local.project_name}-frontend-app-client-development"].s3_bucket_website_endpoint}/{file}"
      timeout_milliseconds = 30 * 1000
      integration_method   = "GET"
    }
    "GET /console" = {
      description          = "S3 frontend proxy"
      integration_type     = "HTTP_PROXY"
      integration_uri      = "http://${module.s3_bucket_development_frontend["${local.project_name}-frontend-app-admin-development"].s3_bucket_website_endpoint}/"
      timeout_milliseconds = 30 * 1000
      integration_method   = "GET"
    }
    "GET /console/{file+}" = {
      description          = "S3 frontend proxy"
      integration_type     = "HTTP_PROXY"
      integration_uri      = "http://${module.s3_bucket_development_frontend["${local.project_name}-frontend-app-admin-development"].s3_bucket_website_endpoint}/{file}"
      timeout_milliseconds = 30 * 1000
      integration_method   = "GET"
    }
    "GET /media/{file+}" = {
    ...
    "GET /{file+}" = {
      description          = "S3 frontend proxy"
      integration_type     = "HTTP_PROXY"
      integration_uri      = "http://${module.s3_bucket_development_frontend["${local.project_name}-frontend-landing-development"].s3_bucket_website_endpoint}/{file}"
      timeout_milliseconds = 30 * 1000
      integration_method   = "GET"
    }
  }
  ...
  authorizers = {
    "auth-function" = {
      ...
      authorizer_uri                    = data.aws_lambda_function.development_backend.invoke_arn
